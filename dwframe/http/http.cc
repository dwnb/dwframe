...
namespace http {
    ...
    std::shared_ptr<HttpResponse> HttpRequest::createResponse() {
        HttpResponse::pointer rsp(new HttpResponse(getVersion()
                                ,isClose()));
        return rsp;
    }
    ...
    #define PARSE_PARAM(str, m, flag, trim) \
        ...
        do { \
            ...
            m.insert(std::make_pair(trim(str.substr(last, key - last)), \
                        dwframe::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1)))); \
            if(pos == std::string::npos) { \
                break; \
            } \
            ++pos; \
    ...
    void HttpRequest::initCookies() {
        ...
        //PARSE_PARAM(cookie, m_cookies, ';', dwframe::StringUtil::Trim);
        m_parserParamFlag |= 0x4;
    }
    ...
    void HttpResponse::setCookie(const std::string& key, const std::string& val,
        ...
        ss << key << "=" << val;
        if(expired > 0) {
            //ss << ";expires=" << dwframe::Time2Str(expired, "%a, %d %b %Y %H:%M:%S") << " GMT";
        }
        if(!domain.empty()) {
            ss << ";domain=" << domain;
        }
