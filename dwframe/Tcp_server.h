...
struct TcpServerConf {
    using pointer=std::shared_ptr<TcpServerConf>;
...
public:
     ...
     */
    TcpServer(dwframe::IOManager* worker = dwframe::IOManager::GetThis()
              ,dwframe::IOManager* io_woker = dwframe::IOManager::GetThis()
     ...
     * @return 返回是否绑定成功
     */
    virtual bool bind(dwframe::Address::pointer addr, bool ssl = false);
     ...
     * @return 是否绑定成功
     */
    virtual bool bind(const std::vector<Address::pointer>& addrs
    ...
    TcpServerConf::pointer getConf() const { return m_conf;}
    void setConf(TcpServerConf::pointer v) { m_conf = v;}
    void setConf(const TcpServerConf& v);

    virtual std::string toString(const std::string& prefix = "");

    std::vector<Socket::pointer> getSocks() const { return m_socks;}
...
protected:
     ...
     * @brief 处理新连接的Socket类
     */
    virtual void handleClient(Socket::pointer client);

    /**
     * @brief 开始接受连接
     */
    virtual void startAccept(Socket::pointer sock);
protected:
    /// 监听Socket数组
    std::vector<Socket::pointer> m_socks;
    /// 新连接的Socket工作的调度器
    IOManager* m_worker;
